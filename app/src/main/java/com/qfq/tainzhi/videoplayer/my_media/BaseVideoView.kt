...
class BaseVideoView : FrameLayout {
    ...
    var mRenderCallBack: IRenderView.IRenderCallback = object : IRenderView.IRenderCallback {
...
// }
    ...
    fun onPrepared() { // 	Log.i(TAG, "onPrepared " + " [" + this.hashCode() + "] ");
...
// }
    fun onInfo(what: Int, extra: Int) { // 	Log.d(TAG, "onInfo what - " + what + " extra - " + extra);
...
// 	}
    ...
    fun onAutoCompletion() { // 	Runtime.getRuntime().gc();
...
// }
    ...
    fun setRenderView() {
        ...
        val layoutParams = LayoutParams(
                ...
                ViewGroup.LayoutParams.MATCH_PARENT,
                Gravity.CENTER)
        addView(renderUIView, layoutParams)
    }
...
// }
    fun onVideoSizeChanged(width: Int, height: Int) {
        ...
        if (mRenderView != null) { // if (videoRotation != 0) {
...
// }
//
    fun setBufferProgress(bufferProgress: Int) { // 	if (bufferProgress != 0)
...
// }
    companion object {
        ...
        var CURRENT_VIDEO_VIEW: BaseVideoView? = null
            ...
            override fun onAudioFocusChange(focusChange: Int) {
                when (focusChange) {
                    ...
                    AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> {
                        try {
                            val player = CURRENT_VIDEO_VIEW
                            if (player != null && player.state == STATE_PLAYING) { // player.startButton.performClick();
                            }
                        } catch (e: IllegalStateException) {
                            e.printStackTrace()
                        }
        ...
        const val MEDIA_TYPE_SYSTEM = 0
        const val MEDIA_TYPE_IJK = 1
        const val MEDIA_TYPE_EXO = 2
        const val RENDER_TYPE_SURFACE_VIEW = 0
        const val RENDER_TYPE_TEXTURE_VIEW = 1
        fun goOnPlayOnResume() {
            if (CURRENT_VIDEO_VIEW != null) {
                if (CURRENT_VIDEO_VIEW!!.state == STATE_PAUSE) {
                    if (ON_PLAY_PAUSE_TMP_STATE == STATE_PAUSE) {
                        CURRENT_VIDEO_VIEW!!.onStatePause()
                        CURRENT_VIDEO_VIEW!!.mediaInterface!!.pause()
                    } else { // CURRENT_VIDEO_VIEW.onStatePlaying();
                        CURRENT_VIDEO_VIEW!!.mediaInterface!!.start()
                    }
        ...
        fun goOnPlayOnPause() {
            if (CURRENT_VIDEO_VIEW != null) {
                if (CURRENT_VIDEO_VIEW!!.state == STATE_AUTO_COMPLETE || CURRENT_VIDEO_VIEW!!.state == STATE_NORMAL || CURRENT_VIDEO_VIEW!!.state == STATE_PREPARING || CURRENT_VIDEO_VIEW!!.state == STATE_ERROR) {
                    releaseAllVideos()
                } else {
                    ON_PLAY_PAUSE_TMP_STATE = CURRENT_VIDEO_VIEW!!.state
                    CURRENT_VIDEO_VIEW!!.onStatePause()
                    CURRENT_VIDEO_VIEW!!.mediaInterface!!.pause()
                }
            }
...
// 	startFullscreenDirectly(context, _class, new DataSource(url, title));
// }
...
// }
        fun releaseAllVideos() {
            ...
            if (CURRENT_VIDEO_VIEW != null) { // CURRENT_VIDEO_VIEW.reset();
